...
class TechnicalDetail(BaseModel):
    ...
    use_cases: List[str] = Field(default_factory=list, description="Practical use cases")
    code_examples: List[str] = Field(default_factory=list, description="Related code snippets")
    related_technologies: List[str] = Field(default_factory=list, description="Related technologies")
...
class SynthesizedKnowledge(BaseModel):
    ...
    original_knowledge: KnowledgeBase = Field(..., description="Original knowledge base")
...
class InformationExtractor:
    ...
    def __init__(self):
        ...
        self.problem_indicators = [
            'issue', 'problem', 'challenge', 'difficulty', 'error', 'bug',
            'limitation', 'drawback', 'bottleneck', 'pain point'
        ]
        
        self.solution_indicators = [
            'solution', 'fix', 'resolve', 'approach', 'method', 'technique',
            'workaround', 'implementation', 'best practice', 'recommendation'
        ]
    
    async def extract_technical_concepts(self, research_files: List[ResearchFile]) -> List[TechnicalConcept]:
        ...
        for file_data in research_files:
            ...
            for category, pattern in self.tech_patterns.items():
                ...
                for match in matches:
                    ...
                    contexts = re.findall(context_pattern, content, re.IGNORECASE | re.DOTALL)
                    concept_data[concept_name]['contexts'].extend(contexts[:3])  # Limit contexts
    ...
    async def extract_problem_solution_pairs(self, research_files: List[ResearchFile]) -> List[Dict[str, Any]]:
        ...
        for file_data in research_files:
            ...
            for i, paragraph in enumerate(paragraphs):
                ...
                has_problem = any(indicator in paragraph_lower for indicator in self.problem_indicators)
                
                if has_problem:
                    # Look for solution in the same or next few paragraphs
                    solution_candidates = paragraphs[i:i+3]  # Current + next 2 paragraphs
                    
                    for candidate in solution_candidates:
                        candidate_lower = candidate.lower()
                        has_solution = any(indicator in candidate_lower for indicator in self.solution_indicators)
                        
                        if has_solution and candidate != paragraph:
                            pairs.append({
                                'problem': paragraph[:300],  # Truncate for readability
    ...
    async def extract_code_examples(self, research_files: List[ResearchFile]) -> List[CodeExample]:
        ...
        for file_data in research_files:
            if file_data.file_type == 'markdown':
                ...
                code_blocks = self._extract_markdown_code_blocks(file_data.content, str(file_data.path))
                code_examples.extend(code_blocks)
        
        self.logger.info(f"Extracted {len(code_examples)} code examples")
        return code_examples
    
    def _extract_markdown_code_blocks(self, content: str, source_file: str) -> List[CodeExample]:
        ...
        for match in matches:
            ...
            explanation = self._find_code_explanation(content, match.start(), match.end())
    ...
    def _find_code_explanation(self, content: str, code_start: int, code_end: int) -> str:
    ...
    def _assess_code_complexity(self, code: str, language: str) -> str:
        ...
        complexity_indicators = {
            ...
            'advanced': ['async', 'await', 'class', 'interface', 'decorator', 'lambda', 'generator']
        }
        
        code_lower = code.lower()
        
        # Count advanced patterns
        advanced_score = sum(1 for pattern in complexity_indicators['advanced'] if pattern in code_lower)
...
class InformationSynthesizer:
    ...
    async def synthesize_knowledge(self, knowledge_base: KnowledgeBase, research_files: List[ResearchFile]) -> SynthesizedKnowledge:
        """
        ...
        try:
            ...
            technical_concepts = await self.extractor.extract_technical_concepts(research_files)
            
            # Extract problem-solution pairs
            problem_solutions = await self.extractor.extract_problem_solution_pairs(research_files)
            
            # Extract code examples
            code_examples = await self.extractor.extract_code_examples(research_files)
            
            # Generate technical details
            technical_details = await self._generate_technical_details(technical_concepts, research_files)
            
            # Identify key themes
            key_themes = await self._identify_key_themes(knowledge_base.insights, technical_concepts)
            ...
            self.logger.info(f"Knowledge synthesis completed with confidence: {confidence:.2f}")
            return synthesized
    ...
    async def _generate_technical_details(self, concepts: List[TechnicalConcept], research_files: List[ResearchFile]) -> List[TechnicalDetail]:
        ...
        for concept in concepts[:15]:  # Limit to top 15 concepts
            ...
            if relevant_content:
                ...
                description = self._extract_concept_description(concept, relevant_content)
    ...
    def _extract_concept_description(self, concept: TechnicalConcept, content_list: List[str]) -> str:
        ...
        for content in content_list[:3]:  # Limit to first 3 relevant files
            ...
            for sentence in sentences:
                if concept.name.lower() in sentence.lower() and len(sentence.split()) > 5:
    ...
    def _assess_concept_difficulty(self, concept_name: str) -> str:
        ...
        advanced_concepts = ['kubernetes', 'microservices', 'tensorflow', 'blockchain', 'machine learning']
    ...
    async def _identify_key_themes(self, insights: List[Insight], concepts: List[TechnicalConcept]) -> List[str]:
        ...
        common_words = {'this', 'that', 'with', 'from', 'they', 'have', 'been', 'will', 'when', 'where'}
    ...
    async def _extract_best_practices(self, research_files: List[ResearchFile]) -> List[str]:
        """Extract best practices from research content."""
        practices = []
        
        practice_indicators = [
            'best practice', 'recommendation', 'should', 'avoid', 'consider',
            'tip:', 'note:', 'important:', 'warning:', 'remember'
        ]
    ...
    def _calculate_synthesis_confidence(
        ...
        if concepts:
            avg_concept_importance = sum(c.importance_score for c in concepts) / len(concepts)
            factors.append(min(1.0, avg_concept_importance * 2))
        
        # Content richness factor
        content_richness = min(1.0, (len(concepts) + len(problem_solutions) + len(code_examples)) / 20)
        factors.append(content_richness)
        
        # Technical detail quality factor
        if technical_details:
            avg_detail_confidence = sum(td.confidence_score for td in technical_details) / len(technical_details)
            factors.append(avg_detail_confidence)
        
        # Problem-solution coverage
        ps_coverage = min(1.0, len(problem_solutions) / 5)  # Ideal: 5+ problem-solution pairs
