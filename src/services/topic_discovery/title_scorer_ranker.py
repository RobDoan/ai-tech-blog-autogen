...
class TitleScorerRanker:
    ...
    def __init__(self,
        ...
        if abs(weight_sum - 1.0) > 0.01:
            self.logger.warning(f"Scoring weights sum to {weight_sum:.3f}, should be 1.0")
    ...
    def _compile_scoring_patterns(self):
        ...
        self.specificity_patterns = {
            'metrics': re.compile(r'\b(\d+(?:\.\d+)?%|\d+(?:\.\d+)?\s*(?:ms|seconds?|x|times?|MB|GB))\b', re.IGNORECASE),
            'versions': re.compile(r'\b(?:v?\d+\.\d+(?:\.\d+)?|[A-Z][a-z]+\s+\d+(?:\.\d+)?)\b'),
            'companies': re.compile(r'\b(Netflix|Google|Amazon|AWS|Facebook|Meta|Microsoft|Apple|Stripe|Uber|Airbnb|Spotify|Twitter|X|LinkedIn|GitHub|GitLab|Vercel|Netlify|Cloudflare|OpenAI|Anthropic)\b'),
            'technologies': re.compile(r'\b(React|Vue|Angular|Next\.js|Node\.js|Python|JavaScript|TypeScript|Go|Rust|Java|C\+\+|C#|Swift|Kotlin|Docker|Kubernetes|PostgreSQL|MongoDB|Redis|GraphQL|REST|API)\b'),
            'specific_numbers': re.compile(r'\b\d+\s+(?:ways?|steps?|methods?|techniques?|tips?|tricks?|strategies?)\b', re.IGNORECASE)
        }

        # Engagement indicators (action-oriented, problem-solving)
        self.engagement_patterns = {
            'action_words': re.compile(r'\b(how|why|what|when|where|guide|tutorial|learn|master|build|create|implement|optimize|improve|solve|fix)\b', re.IGNORECASE),
            'results': re.compile(r'\b(improved?|reduced?|increased?|optimized?|enhanced?|better|faster|easier|simpler|efficient)\b', re.IGNORECASE),
            'problems': re.compile(r'\b(problem|issue|challenge|bug|error|bottleneck|slow|broken|failing|difficult)\b', re.IGNORECASE),
            'benefits': re.compile(r'\b(save|gain|boost|accelerate|streamline|automate|scale|secure)\b', re.IGNORECASE)
        }

        # SEO-friendly patterns
        self.seo_patterns = {
            'questions': re.compile(r'\b(how|why|what|when|where|which|should|can|will|do|does|is|are)\b', re.IGNORECASE),
            'lists': re.compile(r'\b\d+\s+(?:best|top|ways?|tips?|methods?|tools?|frameworks?|libraries?)\b', re.IGNORECASE),
            'comparisons': re.compile(r'\bvs\.?|\bcompared?\s+to\b|\bbetter\s+than\b|\bversus\b', re.IGNORECASE)
        }
    ...
    def _load_engagement_keywords(self):
        ...
        self.engagement_keywords = {
            'developers': [
                'api', 'framework', 'library', 'database', 'performance', 'optimization',
                'debugging', 'testing', 'deployment', 'architecture', 'microservices',
                'containerization', 'ci/cd', 'devops', 'monitoring', 'logging'
            ],
            'managers': [
                'productivity', 'efficiency', 'cost', 'roi', 'scaling', 'team',
                'process', 'workflow', 'automation', 'strategy', 'planning',
                'resources', 'budget', 'timeline', 'delivery'
            ],
            'beginners': [
                'introduction', 'getting started', 'basics', 'fundamentals', 'tutorial',
                'guide', 'step by step', 'learn', 'understand', 'explain',
                'simple', 'easy', 'beginner', 'first time'
            ]
        }
    ...
    def _load_technical_authority_indicators(self):
        ...
        self.authority_indicators = {
            'high_authority_companies': [
                'Netflix', 'Google', 'Amazon', 'Facebook', 'Microsoft', 'Apple',
                'Stripe', 'Uber', 'Airbnb', 'Spotify', 'GitHub'
            ],
            'technical_depth_indicators': [
                'architecture', 'implementation', 'deep dive', 'internals', 'optimization',
                'performance', 'scalability', 'distributed', 'system design'
            ],
            'innovation_indicators': [
                'new', 'latest', 'cutting-edge', 'innovative', 'breakthrough',
                'revolutionary', 'next-generation', 'state-of-the-art'
            ]
        }
    ...
    def rank_titles(self, titles: list[BlogTitleCandidate]) -> list[RankedBlogTitle]:
        ...
        filtered_titles = [
            title for title in ranked_titles
        ...
        if len(filtered_titles) < len(ranked_titles):
            ...
            self.logger.info(f"Filtered out {filtered_count} titles below specificity threshold")

        # Sort by ranking score
        sorted_titles = sorted(filtered_titles, key=lambda x: x.scores.ranking_score, reverse=True)

        # Assign ranks and percentiles
        for i, title in enumerate(sorted_titles):
            title.rank = i + 1
            title.percentile = 100 * (len(sorted_titles) - i) / len(sorted_titles) if sorted_titles else 0
            title.quality_tier = self._determine_quality_tier(title.scores.ranking_score)

        self.logger.info(f"Ranked {len(sorted_titles)} qualifying titles")
        return sorted_titles

    def _calculate_comprehensive_scores(self, candidate: BlogTitleCandidate) -> TitleScores:
        ...
        # 1. Specificity Score
        scores.specificity_score = self._calculate_specificity_score(candidate, title_text)

        # 2. Engagement Score
        scores.engagement_score = self._calculate_engagement_score(candidate, title_text)
        ...
        # 5. Uniqueness Score
        scores.uniqueness_score = self._calculate_uniqueness_score(candidate, title_text)
    ...
    def _calculate_specificity_score(self, candidate: BlogTitleCandidate, title_text: str) -> float:
        ...
        for pattern_type, pattern in self.specificity_patterns.items():
            ...
            if pattern_type == 'metrics':
                score += min(matches * 0.3, 0.4)  # Performance metrics are highly valuable
    ...
    def _calculate_engagement_score(self, candidate: BlogTitleCandidate, title_text: str) -> float:
        ...
        if audience in self.engagement_keywords:
            keyword_matches = sum(
                1 for keyword in self.engagement_keywords[audience]
    ...
    def _calculate_recency_score(self, candidate: BlogTitleCandidate) -> float:
        ...
        trending_techs = [
            ...
            'react 19', 'node 20', 'python 3.12', 'typescript 5',
        ...
        if any(pattern.search(candidate.title) for pattern in [
    ...
    def _calculate_authority_score(self, candidate: BlogTitleCandidate, title_text: str) -> float:
        ...
        depth_matches = sum(
            1 for indicator in self.authority_indicators['technical_depth_indicators']
        ...
        innovation_matches = sum(
            1 for indicator in self.authority_indicators['innovation_indicators']
            if indicator in title_text
        )
    ...
    def _calculate_uniqueness_score(self, candidate: BlogTitleCandidate, title_text: str) -> float:
    ...
    def _calculate_seo_score(self, candidate: BlogTitleCandidate, title_text: str) -> float:
    ...
    def _apply_ranking_adjustments(self, scores: TitleScores, candidate: BlogTitleCandidate) -> float:
    ...
    def _populate_score_details(self, scores: TitleScores, candidate: BlogTitleCandidate):
        ...
        if scores.specificity_score < 0.5:
            scores.improvement_suggestions.append("Add more specific metrics, company names, or version numbers")
        if scores.engagement_score < 0.5:
            scores.improvement_suggestions.append("Include more action words or problem-solving language")
        if scores.seo_score < 0.5:
            scores.improvement_suggestions.append("Consider question format or list structure for better SEO")
    ...
    def _analyze_competitive_keywords(self, ranked_title: RankedBlogTitle):
        ...
        competitive_keywords = [
            ...
            if len(word) > 3 and word not in ['with', 'using', 'from', 'than', 'that', 'this', 'they', 'them', 'their']
        ]
    ...
    def _estimate_audience_size(self, ranked_title: RankedBlogTitle):
        ...
        large_audience_keywords = [
            'javascript', 'python', 'react', 'api', 'how', 'guide', 'tutorial'
        ]

        # Niche audience indicators
        niche_audience_keywords = [
            'kubernetes', 'graphql', 'rust', 'scala', 'architecture', 'optimization'
        ]

        large_matches = sum(1 for keyword in large_audience_keywords if keyword in title_lower)
        niche_matches = sum(1 for keyword in niche_audience_keywords if keyword in title_lower)
    ...
    def get_scoring_summary(self, ranked_titles: list[RankedBlogTitle]) -> dict[str, Any]:
        ...
        summary = {
            ...
            "top_patterns": Counter(t.original_candidate.pattern_type for t in ranked_titles).most_common(3),
            "common_improvements": []
        }
