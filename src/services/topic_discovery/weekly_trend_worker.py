...
class WeeklyTrendWorker:
    ...
    def _init_s3_client(self):
        ...
        try:
            if aws_access_key_id and aws_secret_access_key:
                ...
                self.s3_client = boto3.client('s3', region_name=aws_region or 'us-east-1')
    ...
    async def run_weekly_discovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Collected external trend data")
            
            # Step 3: Aggregate and score all trends
            self.logger.info("Phase 3: Aggregating and scoring trends")
            aggregated_trends = await self._aggregate_and_score_trends(news_trends, external_trends)
            ...
            self.logger.info(f"Weekly trend discovery completed successfully in {results['execution_duration']:.1f}s")
    ...
    async def _collect_external_trends(self) -> Dict:
        ...
        try:
            ...
            external_data = await loop.run_in_executor(None, self.trend_spotter.get_weekly_trend)
    ...
    async def _aggregate_and_score_trends(self, news_trends: List[Dict], external_trend: Dict) -> List[Dict]:
        ...
        """
        self.logger.info(f"Aggregating {len(news_trends)} news trends with external data")
        ...
        for topic_key, trend_data in trend_map.items():
            ...
            if final_score >= self.config['score_threshold']:
                trend_entry = {
                    ...
                    'discovered_at': trend_data['discovered_at'].isoformat() if hasattr(trend_data['discovered_at'], 'isoformat') else str(trend_data['discovered_at']),
                    'duplicate_flag': trend_data['duplicate_flag']
                }
                aggregated_trends.append(trend_entry)
        
        # Sort by final score and limit results
        sorted_trends = sorted(aggregated_trends, key=lambda x: x['score'], reverse=True)
    ...
    async def _upload_to_s3(self, csv_file_path: Path) -> Optional[str]:
        """
