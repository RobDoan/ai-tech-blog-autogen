...
class AIFallbackHandler:
    ...
    def _initialize_keyword_extractors(self):
        ...
        self.extraction_patterns = {
            "companies": re.compile(
                r"\b(Netflix|Google|Amazon|Facebook|Microsoft|Apple|Stripe|Uber|Airbnb|Spotify|GitHub|OpenAI|Anthropic)\b",
                re.IGNORECASE,
            ),
            "technologies": re.compile(
                r"\b(React|Vue|Angular|Python|JavaScript|TypeScript|Docker|Kubernetes|AWS|Azure|API|GraphQL|REST)\b",
                re.IGNORECASE,
            ),
            "metrics": re.compile(
                r"\b(\d+(?:\.\d+)?%|\d+x\s+faster|\d+(?:\.\d+)?\s*(?:ms|seconds?|MB|GB))\b",
                re.IGNORECASE,
            ),
            "versions": re.compile(
            ...
            "action_words": re.compile(
                r"\b(how|why|implement|build|create|optimize|improve|solve|fix|debug)\b",
                re.IGNORECASE,
            ),
        }
    ...
    async def with_fallback(
        ...
        **kwargs,
        ...
        for attempt in range(self.retry_config.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    ...
                    self.logger.info(
                        f"Retrying {operation_name} (attempt {attempt + 1}) after {delay:.1f}s"
                    )
                ...
                self.metrics.avg_response_time = self._update_avg_response_time(
                ...
                self.logger.warning(
                    f"{operation_name} attempt {attempt + 1} failed: {error_type.value}"
                )
        ...
        # Execute fallback
        self.logger.error(
            f"{operation_name} failed after all retries, using fallback. Last error: {str(last_error)}"
        )
        self.metrics.fallback_activations += 1
        ...
        try:
            ...
            raise RuntimeError(
                f"Both {operation_name} and fallback failed"
            ) from last_error
    ...
    def _is_circuit_open(self) -> bool:
        ...
        if (
            ...
            and time.time() - self.circuit_open_time > self.circuit_breaker_timeout
        ):
            self.logger.info(
    ...
    def _check_circuit_breaker(self):
        ...
        if (
            ...
            self.logger.error(
                f"Opening circuit breaker after {self.consecutive_failures} consecutive failures"
            )
    ...
    def fallback_semantic_analysis(
        self, articles: list[ArticleContent]
        ...
        for article in articles:
            try:
                ...
                self.logger.error(
                    f"Error in fallback semantic analysis for article '{article.title}': {str(e)}"
                )

        return insights

    def _create_fallback_semantic_insight(
        ...
        content_text = (
            f"{article.title} {article.summary} {article.full_content or ''}".lower()
        )
        ...
        if metrics:
            key_insights.append(
                f"Performance improvements mentioned: {', '.join(metrics[:2])}"
            )
        if companies:
    ...
    def fallback_title_generation(
        self, insights: list[SemanticInsight], max_titles: int = 20
            ...
            for insight in insights[:10]:  # Limit insights to process
                ...
                for pattern_type, templates in self.title_templates.items():
                    for template in templates[:2]:  # Use top 2 templates per pattern
                        try:
                            title = self._generate_template_title(
                                template, technologies, companies, pattern_type
                            )
                            if title:
    ...
    def _extract_companies_from_insight(self, insight: SemanticInsight) -> list[str]:
        ...
        for insight_text in insight.key_insights:
            company_matches = self.extraction_patterns["companies"].findall(
                insight_text
            )
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        fallback_rate = (
            (self.metrics.fallback_activations / total_calls * 100)
            if total_calls > 0
            else 0
        )
        ...
        return {
            ...
            "timing": {
                ...
                "avg_fallback_response_time": round(
                    self.metrics.fallback_response_time, 3
                ),
            },
            "circuit_breaker": {
            ...
            "cost_estimates": {
                ...
                "estimated_savings_from_fallback": round(
                    self.metrics.cost_savings_from_fallback, 4
                ),
            },
        }
