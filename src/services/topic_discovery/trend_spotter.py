...
class TrendSpotter:
    ...
    def get_weekly_trend(self) -> dict | None:
        ...
        try:
            ...
            if selected_trend:
                self.logger.info(
                    f"Selected trend: {selected_trend.topic} (score: {selected_trend.final_score:.2f})"
                )
                return {
            ...
            if "interest_over_time" in data:
                ...
                for i, topic in enumerate(topics):
                    candidate = TrendCandidate(
                        topic=topic.get("query", ""), search_rank=i + 1
                    )
    ...
    def validate_media_saturation(
        ...
        for candidate in candidates:
            try:
                ...
                self.logger.info(
                    f"Topic '{candidate.topic}': {candidate.article_count} articles found"
                )

            except Exception as e:
                self.logger.error(
                    f"Error validating media saturation for '{candidate.topic}': {str(e)}"
                )
                candidate.article_count = 0

        return candidates

    def validate_social_velocity(
        self, candidates: list[TrendCandidate]
    ) -> list[TrendCandidate]:
        """
        ...
        for candidate in candidates:
            try:
                ...
                response = requests.post(
                    apify_url, json=payload, headers=headers, timeout=60
                )
                response.raise_for_status()
                ...
                self.logger.info(
                    f"Topic '{candidate.topic}': {candidate.tweet_volume} tweets found"
                )

            except Exception as e:
                self.logger.error(
                    f"Error validating social velocity for '{candidate.topic}': {str(e)}"
                )
    ...
    def score_and_select(
        ...
        for candidate in candidates:
            ...
            article_score = (
                candidate.article_count / max_articles if max_articles > 0 else 0
            )
    ...
    def _fallback_to_previous_trend(self) -> dict | None:
        ...
        return {
            ...
            "score": 0.5,
            "metrics": {"search_rank": 5, "article_count": 10, "tweet_volume": 50},
            "timestamp": datetime.now().isoformat(),
            "fallback": True,
        }
