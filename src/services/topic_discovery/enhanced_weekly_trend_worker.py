...
class EnhancedWeeklyTrendWorker:
    ...
    def __init__(self, config: dict | None = None, openai_api_key: str | None = None):
        ...
        self.config.update(
            {
                "max_blog_titles_per_run": self.config.get(
                    "max_blog_titles_per_run", 30
                ),
                "min_specificity_threshold": self.config.get(
                    "min_specificity_threshold", 0.4
                ),
                "enable_ai_analysis": self.config.get("enable_ai_analysis", True),
                "enable_pattern_detection": self.config.get(
                    "enable_pattern_detection", True
                ),
                "content_extraction_timeout": self.config.get(
                    "content_extraction_timeout", 300
                ),  # 5 minutes
                "ai_analysis_timeout": self.config.get(
        ...
        self.content_extractor = EnhancedContentExtractor(timeout=30, max_concurrent=3)
    ...
    async def run_enhanced_discovery(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Extracted {len(article_contents)} articles with enhanced content"
            )
            ...
            if self.config["enable_ai_analysis"] and article_contents:
                ...
                self.logger.info(
                    f"Generated {len(semantic_insights)} semantic insights"
                )
            ...
            if semantic_insights:
                ...
                blog_title_candidates = await self._run_title_generation(
                    semantic_insights
                )
                self.logger.info(
                    f"Generated {len(blog_title_candidates)} blog title candidates"
                )
            ...
            if ranked_titles:
                self.logger.info(
                    "Phase 5: Context enrichment and supporting information"
                )
                enriched_titles = await self._run_context_enrichment(
                    ranked_titles, semantic_insights
                )
                self.logger.info(
                    f"Enriched {len(enriched_titles)} titles with comprehensive context"
                )
            ...
            if enriched_titles:
                self.logger.info(
                    "Phase 7: Enhanced CSV export with comprehensive context"
                )
                csv_file_path = await self._export_enhanced_csv(
                    enriched_titles, pattern_analysis
                )
            ...
            self.logger.info(
                f"Enhanced blog title discovery completed successfully in {results['execution_duration']:.1f}s"
            )
    ...
    async def _run_semantic_analysis(
        ...
        try:
            ...
            selected_articles = self._select_articles_for_ai_analysis(
                article_contents, max_articles_for_ai
            )
    ...
    def _select_articles_for_ai_analysis(
        self, articles: list[ArticleContent], max_count: int
    ) -> list[ArticleContent]:
        ...
        sorted_articles = sorted(
            articles, key=lambda x: x.content_quality_score, reverse=True
        )
    ...
    async def _run_title_generation(
        self, semantic_insights: list[SemanticInsight]
    ) -> list[BlogTitleCandidate]:
        """Run AI blog title generation phase"""

        if not hasattr(self, "title_generator"):
            self.logger.warning(
                "Title generator not available, skipping title generation"
            )
            return []

        try:
            max_titles = self.config["max_blog_titles_per_run"]
            return await self.title_generator.generate_specific_titles(
                semantic_insights, max_titles
            )
        except Exception as e:
            self.logger.error(f"Title generation failed: {str(e)}")
            return []

    def _run_title_scoring(
        self, blog_title_candidates: list[BlogTitleCandidate]
    ) -> list[RankedBlogTitle]:
    ...
    async def _run_context_enrichment(
        ...
        try:
            return await self.context_enricher.enrich_titles_context(
                ranked_titles, semantic_insights
            )
        except Exception as e:
    ...
    async def _run_pattern_detection(
        self, enriched_titles: list[EnrichedBlogTitle]
        ...
        try:
            ...
            connections = await self.pattern_detector.identify_trend_connections(
                enriched_titles
            )
            ...
            analysis = PatternAnalysis(
                ...
                confidence_score=sum(t.scores.ranking_score for t in enriched_titles)
                / len(enriched_titles)
                if enriched_titles
                else 0,
            )
            ...
            analysis.dominant_categories = self._extract_dominant_categories(
                enriched_titles
            )
            analysis.trending_technologies = self._extract_trending_technologies(
                enriched_titles
            )
    ...
    def _extract_trending_technologies(
        self, enriched_titles: list[EnrichedBlogTitle]
    ) -> list[str]:
    ...
    def _identify_content_gaps(
        self, themes: list, enriched_titles: list[EnrichedBlogTitle]
    ) -> list[str]:
        ...
        for title in enriched_titles:
            covered_topics.update(
                [tech.lower() for tech in title.context.key_technologies]
            )
    ...
    async def _export_enhanced_csv(
        self,
            ...
            for enriched_title in enriched_titles:
                ...
                row = {
                    ...
                    "specificity_score": round(
                        enriched_title.ranked_title.scores.specificity_score, 3
                    ),
                    "engagement_score": round(
                        enriched_title.ranked_title.scores.engagement_score, 3
                    ),
                    "overall_score": round(
                        enriched_title.ranked_title.scores.overall_score, 3
                    ),
                    "source_articles": "; ".join(
                        enriched_title.context.source_articles[:3]
                    ),  # Limit to 3 sources
                    "key_technologies": ", ".join(
                        enriched_title.context.key_technologies[:5]
                    ),
                    "supporting_metrics": "; ".join(
                        enriched_title.supporting_details.performance_metrics[:3]
                    ),
                    "content_angle": enriched_title.content_angles[0].angle_type
                    if enriched_title.content_angles
                    else "",
                    ...
                    if enriched_title.ranked_title.scores.overall_score > 0.7
                    else "medium"
                    if enriched_title.ranked_title.scores.overall_score > 0.5
                    else "low",
                    ...
                    "estimated_word_count": enriched_title.content_angles[
                        0
                    ].estimated_word_count
                    if enriched_title.content_angles
                    else 1500,
                    ...
                    "evergreen_potential": round(
                        enriched_title.context.evergreen_potential, 3
                    ),
                }
        ...
        self.logger.info(
            f"Exported {len(enriched_titles)} enhanced blog titles to {csv_file_path}"
        )
        return csv_file_path

    def _compile_analytics(
        self,
        ...
        analytics = {
            "content_extraction": {
                ...
                "avg_content_quality_score": sum(
                    a.content_quality_score for a in article_contents
                )
                / len(article_contents)
                if article_contents
                else 0,
                "articles_with_full_content": sum(
                    1 for a in article_contents if a.full_content
                ),
                "avg_word_count": sum(a.word_count for a in article_contents)
                / len(article_contents)
                if article_contents
                else 0,
            },
            "ai_analysis": {
                ...
                "avg_confidence_score": sum(
                    s.confidence_score for s in semantic_insights
                )
                / len(semantic_insights)
                if semantic_insights
                else 0,
                "total_technical_concepts": sum(
                    len(s.technical_concepts) for s in semantic_insights
                ),
                "total_performance_metrics": sum(
                    len(s.performance_metrics) for s in semantic_insights
                ),
            },
            "title_generation": {
                "total_title_candidates": len(blog_title_candidates),
                "titles_by_pattern": self._analyze_title_patterns(
                    blog_title_candidates
                ),
                "avg_ai_confidence": sum(c.confidence for c in blog_title_candidates)
                / len(blog_title_candidates)
                if blog_title_candidates
                else 0,
            },
            "title_scoring": {
                "titles_ranked": len(ranked_titles),
                "avg_specificity_score": sum(
                    t.scores.specificity_score for t in ranked_titles
                )
                / len(ranked_titles)
                if ranked_titles
                else 0,
                "avg_engagement_score": sum(
                    t.scores.engagement_score for t in ranked_titles
                )
                / len(ranked_titles)
                if ranked_titles
                else 0,
            ...
            "context_enrichment": {
                ...
                "content_angles_suggested": sum(
                    len(t.content_angles) for t in enriched_titles
                ),
                "series_opportunities": sum(
                    len(t.series_potential) for t in enriched_titles
                ),
            },
        }
    ...
    def _analyze_title_patterns(
        self, candidates: list[BlogTitleCandidate]
    ) -> dict[str, int]:
